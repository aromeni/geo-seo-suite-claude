...
REPO_URL="https://github.com/aromeni/geo-seo-suite-claude.git"
...
main() {
    ...
    # ---- Install Hooks ----
    if [ -d "$SOURCE_DIR/hooks" ] && [ -n "$(ls -A "$SOURCE_DIR/hooks" 2>/dev/null)" ]; then
    ...
    if [ -f "$SOURCE_DIR/requirements.txt" ]; then
        if $PYTHON_CMD -m pip install -r "$SOURCE_DIR/requirements.txt" --user -q; then
            print_success "Python dependencies installed"
        else
            print_warning "Some Python dependencies failed to install."
            echo "  Run manually: $PYTHON_CMD -m pip install -r requirements.txt --user"
    ...
    ls "$AGENTS_DIR"/geo-*.md &>/dev/null && print_success "Agent files" || { print_error "Agent files missing"; VERIFY_OK=false; }
    ...
    echo ""
    echo "  Documentation: https://github.com/aromeni/geo-seo-suite-claude"
    echo ""
}
