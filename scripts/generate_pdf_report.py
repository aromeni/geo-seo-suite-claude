...
def create_bar_chart(data, labels, width=400, height=200):
    ...
    # Color each bar individually based on its score
    for i, score in enumerate(data):
        chart.bars[i, 0].fillColor = get_score_color(score)
        chart.bars[i, 0].strokeColor = None
        chart.bars[i, 0].strokeWidth = 0
...
def generate_report(data, output_path="GEO-REPORT.pdf"):
    ...
    elements.append(Paragraph(
        f'<font color="#{score_color.hexval()[2:]}">{get_score_label(geo_score)}</font>',
    ...
    if findings:
        for finding in findings:
            ...
            elements.append(Paragraph(
                f'<font color="#{sev_color.hexval()[2:]}">[{severity}]</font> <b>{title}</b>',
                styles['BodyText_Custom']
            ))
            if description:
